...
def start_new_sim():
    ypr = np.array([np.random.uniform(0, 5.0/180*np.pi), np.random.uniform(0, 5.0/180*np.pi), np.random.uniform(0, 5.0/180*np.pi)])
    dtr = np.array([np.random.uniform(0, 5.0), np.random.uniform(0, 5.0), np.random.uniform(0, 5.0)])
    tra = tr0 + dtr
    rospy.set_param('/uav60/eight_generator/pattern/rotation_ypr', ypr)
    rospy.set_param('/uav60/eight_generator/pattern/translation', tra)

    
def main():
    global tf_buffer, results
    rospy.init_node("mbzirc2020_filter_eval")

    fname = rospy.get_param('~out_fname')
    tr0 = np.array([0.0, 0.0, 12.5])
